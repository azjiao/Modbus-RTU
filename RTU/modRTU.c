...
//串口2接收中断服务函数
//当串口接收到一个字节时发生该中断.
//发生表明:可能是帧起始也可能是帧中间的数据流.
//但无论如何帧起始也是数据流.
//如果帧接收缓冲区的当前索引为0,则视为帧起始.
void USART2_IRQHandler(void)
{
    ...
    if(USART_GetITStatus(USART2, USART_IT_RXNE) == SET)
    {
        ...
        //如果字节接收无错且接收缓冲区没有越限则接收转储数据,并重新启动t1.5定时器.
        //如果通讯出错，则不储存数据到接收缓冲区，将导致CRC校验错误，也会让最终结果错误。
